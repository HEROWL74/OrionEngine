...
namespace Engine::Scripting {
    ...
    void LuaScriptComponent::invalidateCachedFunctions()
    {
        ...
        Utils::log_info(std::format(
            "LuaScriptComponent invalidated successfully: {}", m_scriptPath
        ));
    }
    ...
    void LuaScriptComponent::start()
    {
        ...
        if (m_onStart.valid())
        {
            try
            {
                ...
                if (!resolved)
                {
                    Utils::log_error(Utils::make_error(
                        ...
                        std::format("Failed to resolve GameObject {} in onStart",
                            currentObj->getName())
                    ));
                    return;
                }
                ...
                Utils::log_info(std::format(
                    "Calling onStart for: {}", currentObj->getName()
                ));
            ...
            {
                Utils::log_error(
                    Utils::make_error(Utils::ErrorType::FileI0,
                        std::format("Lua error in onStart ({}): {}",
                            m_scriptPath, e.what())
                    )
                );
                m_onStart = sol::lua_nil;
    ...
    void LuaScriptComponent::update(float deltaTime)
    {
        ...
        // ホットリロード経由で invalidate された時のみ再起動する
        if (m_needsRefresh)
        {
            m_luaStarted = false;
        ...
        try
        {
            ...
            m_onUpdate(resolved, deltaTime);
        }
        catch (const sol::error& e)
        {
            Utils::log_error(
                Utils::make_error(Utils::ErrorType::FileI0,
                    std::format("Lua error in onUpdate ({}): {}",
                        m_scriptPath, e.what())
                )
            );
            m_onUpdate = sol::lua_nil;
        ...
        catch (const std::exception& e)
        {
            Utils::log_error(
                Utils::make_error(Utils::ErrorType::Unknown,
                    std::format("C++ exception in onUpdate ({}): {}",
                        m_scriptPath, e.what())
                )
            );
            m_onUpdate = sol::lua_nil;
