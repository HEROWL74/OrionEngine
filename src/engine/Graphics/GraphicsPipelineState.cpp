...
namespace Engine::Graphics
{
	...
	[[nodiscatd]] Utils::VoidResult GraphicsPipelineState::initialize(
		Device* device,
		RootSignature&& rootSignature,
		const GraphicsPipelineStateDesc& desc)
	{
		CHECK_CONDITION(device != nullptr, Utils::ErrorType::DeviceCreation, "Device is null");
		CHECK_CONDITION(rootSignature.isValid(), Utils::ErrorType::ResourceCreation, "RootSignature is not initialized");
	...
	[[nodiscard]] Utils::VoidResult GraphicsPipelineState::createPSO(
		Device* device,
		const GraphicsPipelineStateDesc& desc
	)
	{
		CHECK_CONDITION(desc.vertexShader && desc.vertexShader->isValid(),
			Utils::ErrorType::ShaderCompilation, "VertexShader is null or invaild");
		CHECK_CONDITION(desc.pixelShader && desc.pixelShader->isValid(),
			Utils::ErrorType::ShaderCompilation, "PixelShader is null or invaild");
		...
		// シェーダー
		psoDesc.VS = { desc.vertexShader->getBytecode(), desc.vertexShader->getBytecodeSize() };
		psoDesc.PS = {desc.pixelShader->getBytecode(), desc.pixelShader->getBytecodeSize()};
		...
		CHECK_HR(device->getDevice()->CreateGraphicsPipelineState(
			...
			Utils::ErrorType::ResourceCreation, std::format("Failed to create PSO: {}", desc.debugName));
