...
namespace Runtime
{
	...
	Engine::Utils::VoidResult GameApp::initD3D()
	{
		...
		// GPU同期してからLua側の初期化を開始
		m_device.waitForGpu();

		// LuaBindings + ScriptManager初期化
		...
		// バインディング登録を関数化（reloadAll時にも再登録されるため）
