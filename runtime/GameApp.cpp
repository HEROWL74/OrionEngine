...
// initialize
// ============================================================
Engine::Utils::VoidResult
GameApp::initialize(HINSTANCE hInstance, int nCmdShow,
...
Engine::Utils::VoidResult GameApp::initD3D() {
  ...
  Renderer::DeviceSettings deviceSettings{.enableDebugLayer = false,
  ...
  auto splashResult = m_splashScreen.initialize(
      &m_device, m_shaderManager.get(), &m_textureManager);
  if (!splashResult) {
    Engine::Utils::log_warning(
        "Failed to initialize splash screen, continuing anyway");
  ...
  if (!inputManager || !inputManager->isInitialized()) {
    return std::unexpected(Engine::Utils::make_error(
        Engine::Utils::ErrorType::Unknown, "InputManager not initialized"));
  }
  ...
  if (m_mainCameraObject == nullptr) {
    ...
    Engine::Utils::log_info(
        "No MainCamera in scene, using default camera position");
  }
...
void GameApp::syncCameraFromScene() {
  ...
  Engine::Utils::log_info(
      std::format("Camera synced from MainCamera (pos: {:.2f}, {:.2f}, {:.2f})",
...
void GameApp::update() {
  ...
  if (m_mainCameraObject && !m_mainCameraObject->isDestroyed()) {
    auto *camComp =
        m_mainCameraObject->getComponent<Engine::World::CameraComponent>();
    if (camComp) {
...
void GameApp::render() {
  ...
  const bool vsync =
      Engine::Core::ProjectSettings::get().getWindowConfig().vsync;
...
Engine::Utils::VoidResult GameApp::createCommandQueue() {
  ...
  CHECK_HR(m_device.getDevice()->CreateCommandQueue(
           ...
           Engine::Utils::ErrorType::DeviceCreation,
...
Engine::Utils::VoidResult GameApp::createRenderTargets() {
  ...
  CHECK_HR(m_device.getDevice()->CreateDescriptorHeap(&rtvHeapDesc,
           ...
           Engine::Utils::ErrorType::ResourceCreation,
