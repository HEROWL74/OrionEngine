...
// IDI_APP_ICON と同じ値。orion_resource.h への依存を排除。
// OrionResourceInjector が POST_BUILD でこの ID にアイコンを注入する。
static constexpr WORD kAppIconId = 101;

namespace Engine::Core {
Window::~Window() { destroy(); }

Utils::VoidResult Window::create(HINSTANCE hInstance,
  ...
  m_handle = CreateWindowExW(0, m_className.c_str(), settings.title.c_str(),
                             windowStyle, settings.x, settings.y, windowWidth,
  ...
  // POST_BUILD で注入済みのアイコンをリソースセクションから取得する。
  // 注入前（初回ビルド直後）は nullptr になるが、CHECK は行わず続行する。
  HICON hIconBig =
      static_cast<HICON>(LoadImageW(m_hInstance, MAKEINTRESOURCEW(kAppIconId),
  ...
  if (hIconBig)
    SendMessageW(m_handle, WM_SETICON, ICON_BIG,
                 reinterpret_cast<LPARAM>(hIconBig));
  if (hIconSmall)
    SendMessageW(m_handle, WM_SETICON, ICON_SMALL,
  ...
  Utils::log_info(
      std::format("Window created: {}x{}", settings.width, settings.height));
...
Utils::VoidResult Window::registerWindowClass(HINSTANCE hInstance) {
  ...
  // 注入済みアイコンを取得。注入前は nullptr → OS デフォルトにフォールバック。
  HICON hIcon =
      static_cast<HICON>(LoadImageW(hInstance, MAKEINTRESOURCEW(kAppIconId),
                                    IMAGE_ICON, 32, 32, LR_DEFAULTCOLOR));
  HICON hIconSm =
      static_cast<HICON>(LoadImageW(hInstance, MAKEINTRESOURCEW(kAppIconId),
                                    IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR));
  ...
  wcex.hIconSm = hIconSm ? hIconSm : LoadIcon(nullptr, IDI_APPLICATION);
...
LRESULT CALLBACK Window::staticWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam,
                                          LPARAM lParam) {
