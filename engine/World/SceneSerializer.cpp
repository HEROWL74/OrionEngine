...
Utils::VoidResult
SceneSerializer::saveScene(const Scene &scene,
...
Utils::VoidResult
SceneSerializer::loadScene(Scene &scene, Renderer::Device *device,
  ...
  try {
    ...
    if (!sceneJson.contains("version")) {
      return std::unexpected(Utils::make_error(
          Utils::ErrorType::Unknown, "Invalid scene file: missing version"));
    }

    if (sceneJson.contains("gameObjects")) {
      for (const auto &objJson : sceneJson["gameObjects"]) {
        auto result =
            deserializeGameObject(scene, device, shaderManager, materialManager,
                                  textureManager, objJson);

        if (!result) {
          Utils::log_warning(std::format("Failed to load GameObject: {}",
    ...
    Utils::log_info(std::format(
        "Scene loaded from: {} ({} objects)", filePath.string(),
...
nlohmann::json
SceneSerializer::serializeGameObject(const Scene &scene,
  ...
  // CameraComponent
  if (auto *camComp =
          batch.get<Engine::World::CameraComponent>(gameObject->getId()))
...
nlohmann::json
SceneSerializer::serializeRenderEntry(const Renderer::RenderEntry &entry,
                                      const Scene & /*scene*/) {
  nlohmann::json j;
...
Utils::VoidResult SceneSerializer::deserializeGameObject(
    Scene &scene, Renderer::Device *device,
    Renderer::ShaderManager *shaderManager,
    Renderer::MaterialManager *materialManager,
    Renderer::TextureManager *textureManager, const nlohmann::json &json) {
  ...
  if (json.contains("renderComponent")) {
    ...
    auto result = deserializeRenderComponent(
        gameObject, scene, device, shaderManager, materialManager,
        textureManager, json["renderComponent"]);
    if (!result)
      return result;
  ...
  if (json.contains("lua")) {
    ...
    if (!scriptPath.empty()) {
      ...
      Utils::log_info(
          std::format("  LuaScriptComponent loaded: {}", scriptPath));
    }
  }
  ...
  if (json.contains("cameraComponent")) {
    Utils::log_info(
        std::format("  [DEBUG] cameraComponent key found for {}", name));
    deserializeCameraComponent(scene, gameObject, json["cameraComponent"]);

    // 追加直後に取得できるか確認
    auto *check = scene.getComponentBatch().get<Engine::World::CameraComponent>(
        gameObject->getId());
    Utils::log_info(
        std::format("  [DEBUG] CameraComponent after deserialize: {}",
                    check ? "OK" : "NULL"));
  }
...
void SceneSerializer::deserializeCameraComponent(Scene &scene,
                                                 Core::GameObject *gameObject,
                                                 const nlohmann::json &json) {
  Utils::log_info(
      std::format("[DEBUG] deserializeCameraComponent called for id=({},{})",
                  gameObject->getId().index, gameObject->getId().generation));

  auto *camComp =
      scene.addComponent<Engine::World::CameraComponent>(gameObject);
...
Utils::VoidResult SceneSerializer::deserializeRenderComponent(
    Core::GameObject *gameObject, Scene &scene, Renderer::Device *device,
    Renderer::ShaderManager *shaderManager,
    Renderer::MaterialManager *materialManager,
    Renderer::TextureManager *textureManager, const nlohmann::json &json) {
  auto renderType =
      static_cast<Renderer::RenderableType>(json.value("renderableType", 0));
  ...
  if (json.contains("material")) {
    ...
    std::string matName =
        gameObject->getName() + "_" + matJson.value("name", "Material");
    ...
    if (material) {
      if (matJson.contains("properties")) {
        ...
        if (propsJson.contains("albedo")) {
          ...
          props.albedo =
              Math::Vector3(albedo[0].get<float>(), albedo[1].get<float>(),
                            albedo[2].get<float>());
        }
      ...
      if (matJson.contains("textures")) {
        ...
        if (textures.contains("albedo")) {
          ...
          if (texture)
            material->setTexture(Renderer::TextureType::Albedo, texture);
        }
      }
    }
  }
...
nlohmann::json
SceneSerializer::serializeUITextComponent(const EngineUI::UIText *text) {
  nlohmann::json j;
  j["text"] = text->getText();
...
void SceneSerializer::deserializeUITextComponent(EngineUI::UIText *text,
                                                 const nlohmann::json &json) {
  ...
  if (json.contains("color")) {
    ...
    text->setColor(Math::Vector3(color[0].get<float>(), color[1].get<float>(),
                                 color[2].get<float>()));
  }
...
nlohmann::json
SceneSerializer::serializeTransform(const Core::Transform *transform) {
  nlohmann::json j;
...
void SceneSerializer::deserializeTransform(Core::Transform *transform,
                                           const nlohmann::json &json) {
...
nlohmann::json
SceneSerializer::serializeMaterial(const Renderer::Material *material) {
  nlohmann::json j;
  j["name"] = "Material";
...
nlohmann::json SceneSerializer::serializeLuaComponent(
  ...
  std::transform(lower.begin(), lower.end(), lower.begin(), ::tolower);
  auto assetsPos = lower.find("assets/"); // ← 途中にあっても検出できる
  if (assetsPos != std::string::npos)
    path = "Assets/" + path.substr(assetsPos + 7);
  j["scriptPath"] = path;
  return j;
}

nlohmann::json
SceneSerializer::serializeBoxCollider(const Physics::BoxCollider *collider) {
  nlohmann::json j;
...
void SceneSerializer::deserializeBoxCollider(Physics::BoxCollider *collider,
                                             const nlohmann::json &json) {
