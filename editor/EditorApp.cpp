...
Utils::VoidResult
EditorApp::initialize(HINSTANCE hInstance, int nCmdShow,
  ...
  if (!projectPath.empty() &&
      std::filesystem::exists(projectPath / "ProjectSettings.json")) {
    // --project 引数あり: そのまま使う
  ...
  Core::WindowSettings windowSettings{.title = settings.getProjectNameW(),
                                      .width = wincfg.width,
  ...
  */
  ...
  while (m_window.processMessages()) {
    ...
    if (c++ % 60 == 0) {
      ...
      Utils::log_info(
          std::format("update={:.3f}ms  render={:.3f}ms  total={:.3f}ms",
...
Engine::Utils::VoidResult EditorApp::initD3D() {
  ...
  Renderer::DeviceSettings deviceSettings{.enableDebugLayer = true,
  ...
  if (!m_shaderManager || !m_shaderManager.get()) {
    ...
    return std::unexpected(
        Utils::make_error(Utils::ErrorType::Unknown,
                          "ShaderManager is null after initialization"));
  }
  ...
  if (!psoCacheResult)
    return psoCacheResult;

  Utils::log_info("PSOCache initialization completed successfully");

  Utils::log_info("Initializing Skybox...");
  auto editorSkyboxResult =
      m_editorSkybox.initialize(&m_device, m_shaderManager.get());
  auto gameSkyboxResult =
      m_gameSkybox.initialize(&m_device, m_shaderManager.get());
  Utils::log_info("Skybox initialized successfully");

  auto textureManagerResult = m_textureManager.initialize(&m_device);
  if (!textureManagerResult)
    return textureManagerResult;

  auto materialManagerResult = m_materialManager.initialize(&m_device);
  if (!materialManagerResult)
    return materialManagerResult;
  ...
  auto uiTextResult =
      m_uiTextRenderer->initialize(&m_device, m_shaderManager.get());
  if (!uiTextResult) {
  ...
  auto editorViewResult = m_editorView.initialize(
      &m_device, clientWidth, clientHeight, m_shaderManager.get(), &m_psoCache);
  if (!editorViewResult) {
  ...
  auto gameViewResult =
      m_gameView.initialize(&m_device, clientWidth, clientHeight, &m_psoCache);
  if (!gameViewResult) {
  ...
  if (std::filesystem::exists(defaultScenePath)) {
    auto loadResult = m_sceneSerializer.loadScene(
        m_scene, &m_device, m_shaderManager.get(), &m_materialManager,
        &m_textureManager, defaultScenePath);
    if (loadResult) {
      ...
      m_currentScenePath = defaultScenePath.string();
      Utils::log_warning(
          "Failed to load default scene, creating initial scene");
      createInitialScene();
    }
  } else {
    m_currentScenePath = defaultScenePath.string();
  ...
  Utils::log_info(
      std::format("Checking for default scene: {}", defaultScenePath.string()));
  ...
  m_playModeController.setSceneLoadContext(
      &m_device, m_shaderManager.get(), &m_materialManager, &m_textureManager,
      m_currentScenePath);
  ...
  m_editorCamera.setPerspective(
      45.0f, static_cast<float>(clientWidth) / clientHeight, 0.1f, 100.0f);
  ...
  m_cameraController =
      std::make_unique<World::FPSCameraController>(&m_editorCamera);
  ...
  m_playModeController.setOnPlayCallback([this]() {
    ...
    for (auto &obj : m_scene.getGameObjects()) {
      if (!obj)
        continue;
      auto *luaComp =
          m_scene.getComponent<Engine::Scripting::LuaScriptComponent>(
              obj.get());
    ...
    Utils::log_info(
        std::format("[Profiler] registered {} components", registered));
  });
  ...
  if (!inputManager) {
    Utils::log_error(Utils::make_error(
        Utils::ErrorType::Unknown, "Failed to get InputManager from Window"));
    return std::unexpected(
        Utils::make_error(Utils::ErrorType::Unknown, "InputManager is null"));
  }
  ...
  Utils::log_info(std::format(
      "Initializing EditorViewWindow with InputManager (initialized: {})",
  ...
  if (auto *mainCamObj = m_scene.findGameObject("MainCamera")) {
    ...
    auto *camComp =
        m_scene.getComponentBatch().get<Engine::World::CameraComponent>(
            mainCamObj->getId());
    Utils::log_info(std::format("[DEBUG] CameraComponent via batch: {}",
                                camComp ? "OK" : "NULL"));
    auto *camCompOld =
        mainCamObj->getComponent<Engine::World::CameraComponent>();
  ...
  m_hierarchyWindow->setSelectionChangedCallback(
      [this](Core::GameObject *object) {
  ...
  m_hierarchyWindow->setCreateObjectCallback(
      [this](UI::PrimitiveType type,
             const std::string &name) -> Core::GameObject * {
        return createPrimitiveObject(type, name);
      });

  m_hierarchyWindow->setDeleteObjectCallback(
      [this](Core::GameObject *object) { deleteGameObject(object); });

  m_hierarchyWindow->setDuplicateObjectCallback(
      [this](Core::GameObject *object) -> Core::GameObject * {
        return duplicateGameObject(object);
      });

  m_hierarchyWindow->setRenameObjectCallback(
      [this](Core::GameObject *object, const std::string &newName) {
        renameGameObject(object, newName);
      });

  m_hierarchyWindow->setCreateUIElementCallback(
      [this](UI::UIElementType type,
             const std::string &name) -> Engine::EngineUI::UIText * {
        return createUIElement(type, name);
      });

  m_hierarchyWindow->setDeleteUITextCallback(
      [this](Engine::EngineUI::UIText *text) { deleteUIText(text); });

  m_hierarchyWindow->setRenameUITextCallback(
      [this](Engine::EngineUI::UIText *text, const std::string &newName) {
        renameUIText(text, newName);
      });

  m_hierarchyWindow->setUISelectionChangedCallback(
      [this](Engine::EngineUI::UIText *text) {
  ...
  m_hierarchyWindow->setCreateGameCameraCallback(
      [this](const std::string &name) -> Core::GameObject * {
...
Engine::Utils::VoidResult EditorApp::initializeInput() {
  ...
  if (!inputManager) {
    return std::unexpected(Utils::make_error(
        Utils::ErrorType::Unknown, "InputManager not available from Window"));
  }
  ...
  Utils::log_info(std::format("InputManager initialized: {}",
                              inputManager->isInitialized()));

  inputManager->setRelativeMouseMode(false);
  inputManager->setMouseSensitivity(0.1f);

  inputManager->setKeyPressedCallback(
      [this](Input::KeyCode key) { onKeyPressed(key); });
  inputManager->setKeyReleasedCallback(
      [this](Input::KeyCode key) { onKeyReleased(key); });
  inputManager->setMouseMoveCallback(
      [this](int x, int y, int deltaX, int deltaY) {
        onMouseMove(x, y, deltaX, deltaY);
      });
  inputManager->setMouseButtonPressedCallback(
      [this](Input::MouseButton button, int x, int y) {
        onMouseButtonPressed(button, x, y);
      });
  inputManager->setMouseButtonReleasedCallback(
      [this](Input::MouseButton button, int x, int y) {
...
Utils::VoidResult EditorApp::createCommandQueue() {
  ...
  CHECK_HR(m_device.getDevice()->CreateCommandQueue(
...
Utils::VoidResult EditorApp::createSwapChain() {
  ...
  CHECK_HR(m_device.getDXGIFactory()->CreateSwapChainForHwnd(
               m_commandQueue.Get(), m_window.getHandle(), &swapChainDesc,
               nullptr, nullptr, &swapChain1),
           Utils::ErrorType::SwapChainCreation, "Failed to create swap chain");

  CHECK_HR(swapChain1.As(&m_swapChain), Utils::ErrorType::SwapChainCreation,
           "Failed to cast swap chain to IDXGISwapChain3");
...
Utils::VoidResult EditorApp::createRenderTargets() {
  ...
  CHECK_HR(m_device.getDevice()->CreateDescriptorHeap(&rtvHeapDesc,
  ...
  for (UINT i = 0; i < 2; i++) {
    ...
    m_device.getDevice()->CreateRenderTargetView(m_renderTargets[i].Get(),
                                                 nullptr, rtvHandle);
...
Utils::VoidResult EditorApp::createDepthStencilBuffer() {
  ...
  CHECK_HR(m_device.getDevice()->CreateDescriptorHeap(&dsvHeapDesc,
...
void EditorApp::render() {
  ...
  m_editorView.render(m_scene, m_commandList.Get(), m_editorCamera,
                      m_frameIndex);
  ...
  if (FAILED(hrClose)) {
    Utils::log_error(Utils::make_error(
        Utils::ErrorType::Unknown,
        std::format("Failed to close command list for 3D rendering: 0x{:08X}",
                    static_cast<unsigned>(hrClose))));
    return;
  }
  ...
  if (firstFrame) {
    Utils::log_info(std::format(
        "First render - EditorCamera: ({:.2f}, {:.2f}, {:.2f})",
  ...
  m_commandList->ClearDepthStencilView(dsvHandle, D3D12_CLEAR_FLAG_DEPTH, 1.0f,
                                       0, 0, nullptr);
  ...
  if (FAILED(hrClose)) {
    Utils::log_error(
        Utils::make_error(Utils::ErrorType::Unknown,
                          std::format("Failed to close command list: 0x{:08X}",
                                      static_cast<unsigned>(hrClose))));
    return;
  }
  ...
  if (FAILED(hr)) {
    Utils::log_warning(
        std::format("Present failed: 0x{:08X}", static_cast<unsigned>(hr)));
    return;
  }
...
void EditorApp::processInput() {
  ...
  bool isSceneWindowHovered =
      m_editorViewWindow && m_editorViewWindow->isHovered();
  ...
  if (allowKeyboardInput) {
    ...
    m_cameraController->processKeyboard(forward, backward, left, right, up,
                                        down, m_deltaTime);
  }

  static bool wasCameraControlActive = false;
  bool cameraControlRequested =
      m_editorViewWindow && m_editorViewWindow->isCameraControlRequested();
  ...
  if (cameraControlRequested) {
    ...
    Utils::log_info(std::format(
        ">>> Camera delta check: X={}, Y={}, RelativeMode={}", deltaX, deltaY,
        inputManager->getMouseState().isRelativeMode));

    if (deltaX != 0 || deltaY != 0) {
      m_cameraController->processMouseMovement(static_cast<float>(deltaX),
...
void EditorApp::waitForPreviousFrame() {
  if (!m_commandQueue || !m_fence || !m_fenceEvent) {
    Utils::log_warning(
        "DirectX objects not initialized in waitForPreviousFrame");
    return;
  }
...
void EditorApp::onWindowResize(int width, int height) {
  Utils::log_info(
      std::format("App::onWindowResize called: {}x{}", width, height));

  if (width <= 0 || height <= 0) {
    Utils::log_warning(
        std::format("Invalid resize dimensions: {}x{}", width, height));
    return;
  }
  ...
  try {
    ...
    if (FAILED(hr)) {
      Utils::log_error(
          Utils::make_error(Utils::ErrorType::SwapChainCreation,
                            std::format("Failed to resize swap chain: 0x{:08x}",
    ...
    auto imguiResult = m_imguiManager.initialize(
        &m_device, m_window.getHandle(), m_commandQueue.Get());
    if (!imguiResult) {
      Utils::log_error(imguiResult.error());
      return;
    }

    m_window.setMessageCallback([&](HWND hwnd, UINT msg, WPARAM w, LPARAM l) {
      return m_imguiManager.handleWindowMessage(hwnd, msg, w, l);
    });
...
void EditorApp::onKeyPressed(Input::KeyCode key) {
  ...
  if (key == Input::KeyCode::Escape) {
    ...
    if (inputManager) {
      ...
      Utils::log_info(
          std::format("Mouse relative mode: {}", !currentMode ? "ON" : "OFF"));
    }
  }
}
...
void EditorApp::onMouseMove(int /*x*/, int /*y*/, int /*deltaX*/,
                            int /*deltaY*/) {}
...
Core::GameObject *EditorApp::createPrimitiveObject(UI::PrimitiveType type,
...
Core::GameObject *EditorApp::duplicateGameObject(Core::GameObject *original) {
  ...
  if (originalTransform && newTransform) {
    newTransform->setPosition(originalTransform->getPosition() +
                              Math::Vector3(1, 0, 0));
...
Engine::EngineUI::UIText *EditorApp::createUIElement(UI::UIElementType type,
                                                     const std::string &name) {
  if (type != UI::UIElementType::Text)
    return nullptr;

  Utils::log_info(
      std::format("EditorApp::createUIElement called with name: {}", name));

  auto *gameObject = m_scene.createGameObject(name);
  if (!gameObject) {
    Utils::log_error(
        Engine::Utils::make_error(Engine::Utils::ErrorType::Unknown,
  ...
  if (!uiText) {
    Utils::log_error(Engine::Utils::make_error(
        Engine::Utils::ErrorType::Unknown, "Failed to add UIText component"));
...
void EditorApp::renameUIText(Engine::EngineUI::UIText *text,
...
Core::GameObject *EditorApp::createGameCamera(const std::string &name) {
  if (m_scene.findGameObject(name) != nullptr) {
    Utils::log_warning(
        std::format("GameCamera '{}' already exists in scene", name));
    return nullptr;
  }
  ...
  if (!camComp) {
    ...
    Utils::log_error(Utils::make_error(
        Utils::ErrorType::Unknown, "Failed to addComponent<CameraComponent>"));
    return nullptr;
  }
...
void EditorApp::renameGameObject(Core::GameObject *object,
                                 const std::string &newName) {
  if (!object)
    return;
...
Renderer::RenderableType
EditorApp::primitiveToRenderableType(UI::PrimitiveType type) {
  switch (type) {
  case UI::PrimitiveType::Cube:
    return Renderer::RenderableType::Cube;
  case UI::PrimitiveType::Sphere:
    return Renderer::RenderableType::Cube;
  case UI::PrimitiveType::Plane:
  case UI::PrimitiveType::Cylinder:
...
UI::PrimitiveType
EditorApp::renderableToPrimitiveType(Renderer::RenderableType renderType) {
  switch (renderType) {
...
void EditorApp::createInitialScene() {
  ...
  std::string texturePath =
      (settings.getAssetRootPath() / "textures/brick_BaseColor.jpg").string();
  std::replace(texturePath.begin(), texturePath.end(), '\\', '/');

  auto cubeTexMat =
      m_materialManager.createMaterial("CubeWithTexture_Material");
  if (cubeTexMat) {
    Renderer::MaterialProperties props;
    props.metallic = 0.0f;
    props.roughness = 0.5f;
...
void EditorApp::createNewScene() {
  ...
  auto &settings = Engine::Core::ProjectSettings::get();
  m_currentScenePath =
      (settings.getAssetRootPath() / "scenes/untitled.scene").string();
...
void EditorApp::saveScene() {
  ...
  if (m_currentScenePath.empty()) {
    auto &settings = Engine::Core::ProjectSettings::get();
    m_currentScenePath =
        (settings.getAssetRootPath() / "scenes/default.scene").string();
    std::replace(m_currentScenePath.begin(), m_currentScenePath.end(), '\\',
                 '/');
  }
  ...
  Utils::log_info(
      std::format("  has_extension: {}", scenePath.has_extension()));
...
void EditorApp::saveSceneAs() {
  ...
  m_currentScenePath = (settings.getAssetRootPath() /
                        std::format("scenes/scene_{}.scene", sceneCounter++))
                           .string();
  std::replace(m_currentScenePath.begin(), m_currentScenePath.end(), '\\', '/');
...
void EditorApp::openScene() {
  ...
  if (result) {
    ...
    if (auto *mainCamObj = m_scene.findGameObject("MainCamera")) {
      if (auto *camComp = m_scene.getComponent<Engine::World::CameraComponent>(
              mainCamObj)) {
        camComp->syncFromTransform();
        m_gameCamera = camComp->getCamera();
        if (m_gameViewWindow)
