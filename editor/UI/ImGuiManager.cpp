...
Utils::VoidResult ImGuiManager::initialize(Device *device, HWND hwnd,
  ...
  CHECK_CONDITION(device->isValid(), Utils::ErrorType::Unknown,
                  "Device is not valid");
  CHECK_CONDITION(commandQueue != nullptr, Utils::ErrorType::Unknown,
  ...
  try {
    ...
    if (!m_context) {
      return std::unexpected(Utils::make_error(
          Utils::ErrorType::Unknown, "Failed to create ImGui context"));
    }
    ...
    if (!ImGui_ImplWin32_Init(hwnd)) {
      ...
      m_context = nullptr;
      return std::unexpected(Utils::make_error(
          Utils::ErrorType::Unknown, "Failed to initialize ImGui Win32"));
    }
    ...
    if (!m_commandQueue) {
      ...
      return std::unexpected(
          Utils::make_error(Utils::ErrorType::Unknown,
                            "CommandQueue became null before DX12 init"));
    }

    if (!ImGui_ImplDX12_Init(
            m_device->getDevice(), static_cast<int>(frameCount), rtvFormat,
            m_srvDescHeap.Get(),
      ...
      m_context = nullptr;
      return std::unexpected(Utils::make_error(
          Utils::ErrorType::Unknown, "Failed to initialize ImGui DX12"));
    }
    ...
    return std::unexpected(Utils::make_error(Utils::ErrorType::Unknown,
                                             "ImGui initialization failed"));
  } catch (...) {
    Utils::log_error(
        Utils::make_error(Utils::ErrorType::Unknown,
                          "Unknown exception during ImGui initialization"));
    shutdown();
    return std::unexpected(Utils::make_error(Utils::ErrorType::Unknown,
                                             "ImGui initialization failed"));
  }
...
Utils::VoidResult ImGuiManager::createFontTextureManually() {
  if (!m_commandQueue) {
    return std::unexpected(
        Utils::make_error(Utils::ErrorType::Unknown,
                          "CommandQueue is null in createFontTextureManually"));
  }

  if (!m_device || !m_device->getDevice()) {
    return std::unexpected(
        Utils::make_error(Utils::ErrorType::Unknown,
                          "Device is null in createFontTextureManually"));
  }
  ...
  try {
    ...
    CHECK_HR(
        ...
        "Failed to create font command allocator");

    CHECK_HR(m_device->getDevice()->CreateCommandList(
                 0, D3D12_COMMAND_LIST_TYPE_DIRECT, commandAllocator.Get(),
                 nullptr, IID_PPV_ARGS(&commandList)),
             Utils::ErrorType::ResourceCreation,
             "Failed to create font command list");

    if (!m_commandQueue) {
      return std::unexpected(Utils::make_error(
          Utils::ErrorType::Unknown,
          "CommandQueue became null before CreateDeviceObjects"));
    }

    if (!ImGui_ImplDX12_CreateDeviceObjects()) {
      Utils::log_warning(
          "ImGui_ImplDX12_CreateDeviceObjects failed, but continuing");
    }
    ...
    if (!m_commandQueue) {
      return std::unexpected(
          Utils::make_error(Utils::ErrorType::Unknown,
                            "CommandQueue became null before execution"));
    }
    ...
    CHECK_HR(m_device->getDevice()->CreateFence(0, D3D12_FENCE_FLAG_NONE,
                                                IID_PPV_ARGS(&fence)),
             Utils::ErrorType::ResourceCreation, "Failed to create font fence");

    HANDLE fenceEvent = CreateEvent(nullptr, FALSE, FALSE, nullptr);
    CHECK_CONDITION(fenceEvent != nullptr, Utils::ErrorType::ResourceCreation,
    ...
    if (!m_commandQueue) {
      ...
      return std::unexpected(Utils::make_error(
          Utils::ErrorType::Unknown, "CommandQueue became null before signal"));
    }
    ...
    return std::unexpected(
        Utils::make_error(Utils::ErrorType::Unknown,
                          "Unknown exception in createFontTextureManually"));
  }
}
...
void ImGuiManager::newFrame() {
  if (!m_initialized || !m_context) {
    Utils::log_error(Utils::make_error(
        Utils::ErrorType::Unknown, "ImGuiManager not initialized in newFrame"));
    return;
  }
...
bool ImGuiManager::handleWindowMessage(HWND hwnd, UINT msg, WPARAM wParam,
                                       LPARAM lParam) const {
  if (!m_initialized || !m_context) {
    return false;
  }
...
Utils::VoidResult ImGuiManager::createDescriptorHeap() {
  ...
  CHECK_HR(m_device->getDevice()->CreateDescriptorHeap(
               &desc, IID_PPV_ARGS(&m_srvDescHeap)),
           Utils::ErrorType::ResourceCreation,
...
void ImGuiManager::onWindowResize(int width, int height) {
  Utils::log_info(
      std::format("ImGuiManager::onWindowResize: {}x{}", width, height));
  ...
  if (width <= 0 || height <= 0) {
    Utils::log_warning(
        std::format("Invalid ImGui resize dimensions: {}x{}", width, height));
    return;
  }
  ...
  try {
    ...
    io.ConfigFlags |=
        ImGuiConfigFlags_ViewportsEnable; // 複数ウィンドウ分離も可能

    if (std::abs(io.DisplaySize.x - width) > 1.0f ||
        std::abs(io.DisplaySize.y - height) > 1.0f) {
      io.DisplaySize =
          ImVec2(static_cast<float>(width), static_cast<float>(height));
      io.DisplayFramebufferScale = ImVec2(1.0f, 1.0f);

      Utils::log_info(
          std::format("ImGui display size updated to: {}x{}", width, height));
    }
  } catch (...) {
    Utils::log_error(Utils::make_error(Utils::ErrorType::Unknown,
                                       "Exception in ImGui resize"));
  }
...
void ImGuiManager::invalidateDeviceObjects() {
  Utils::log_info(
      "invalidateDeviceObjects called - use reinitializeForResize instead");
}

void ImGuiManager::createDeviceObjects() {
  Utils::log_info(
      "createDeviceObjects called - use reinitializeForResize instead");
}
Utils::VoidResult ImGuiManager::reinitializeForResize() {
  ...
  if (!ImGui_ImplDX12_Init(
          m_device->getDevice(), static_cast<int>(m_frameCount), m_rtvFormat,
          m_srvDescHeap.Get(),
          m_srvDescHeap->GetCPUDescriptorHandleForHeapStart(),
          m_srvDescHeap->GetGPUDescriptorHandleForHeapStart())) {
    return std::unexpected(Utils::make_error(
        Utils::ErrorType::Unknown, "Failed to reinitialize ImGui DX12"));
  }
...
ImTextureID ImGuiManager::registerTexture(Texture *tex) {
  if (!tex || !m_device || !m_srvDescHeap || !m_initialized) {
    Utils::log_warning("Cannot register texture - ImGuiManager not properly "
                       "initialized or texture is null");
    return 0;
  }
  ...
  if (m_nextFreeIndex >= m_maxSrv) {
    Utils::log_warning(
        "ImGui descriptor heap is full, cannot register more textures");
    return 0;
  }
  ...
  m_device->getDevice()->CreateShaderResourceView(tex->getResource(), &srv,
                                                  cpu);
  ...
  Utils::log_info(std::format(
      ...
      tex->getDesc().debugName, (int)resDesc.Format, tex->getMipLevels(),
      (uint64_t)cpu.ptr, (uint64_t)gpu.ptr));

  return result;
}

ImTextureID ImGuiManager::registerRenderTarget(ID3D12Resource *texture,
                                               DXGI_FORMAT format) {
  ...
  CD3DX12_CPU_DESCRIPTOR_HANDLE cpuHandle(
      m_srvDescHeap->GetCPUDescriptorHandleForHeapStart(), idx,
  ...
  CD3DX12_GPU_DESCRIPTOR_HANDLE gpuHandle(
      m_srvDescHeap->GetGPUDescriptorHandleForHeapStart(), idx,
      m_srvIncSize // m_descriptorSize ではなく m_srvIncSize を使用
  );
  ...
  Utils::log_info(
      std::format("RegisterRenderTarget: idx={} format={} gpu.ptr=0x{:016X}",
...
void ImGuiManager::createGUIStyle() {
  ...
  colors[ImGuiCol_ButtonHovered] =
      ImVec4(0.25f, 0.40f, 0.65f, 1.0f); // ブルー寄り
...
void SceneHierarchyWindow::drawUIText(Engine::EngineUI::UIText *text,
                                      int index) {
  if (!text)
    return;
...
void SceneHierarchyWindow::setCreateUIElementCallback(
    std::function<Engine::EngineUI::UIText *(UI::UIElementType,
                                             const std::string &)>
        callback) {
  if (m_contextMenu) {
...
void SceneHierarchyWindow::setRenameUITextCallback(
    std::function<void(Engine::EngineUI::UIText *, const std::string &)>
        callback) {
  if (m_contextMenu) {
...
void SceneHierarchyWindow::drawGameObject(Core::GameObject *gameObject,
                                          Core::GameObject *currentSelection) {
  if (!gameObject || gameObject->isDestroyed()) {
    return;
  }
  ...
  if (ImGui::IsItemClicked() && !gameObject->isDestroyed()) {
    ...
    if (uiTextComponent) {
      Utils::log_info(std::format("Selected GameObject with UIText: {}",
                                  gameObject->getName()));
      ...
      Utils::log_info(
          std::format("Selected GameObject: {}", gameObject->getName()));
...
void SceneHierarchyWindow::setSelectionChangedCallback(
    std::function<void(Core::GameObject *)> callback) {
  m_onSelectionChanged = callback;
}

void SceneHierarchyWindow::setCreateObjectCallback(
    std::function<Core::GameObject *(UI::PrimitiveType, const std::string &)>
        callback) {
  if (m_contextMenu) {
...
void SceneHierarchyWindow::setDeleteObjectCallback(
    std::function<void(Core::GameObject *)> callback) {
  if (m_contextMenu) {
...
void SceneHierarchyWindow::setDuplicateObjectCallback(
    std::function<Core::GameObject *(Core::GameObject *)> callback) {
  if (m_contextMenu) {
    m_contextMenu->setDuplicateObjectCallback(callback);
  }
}

void SceneHierarchyWindow::setRenameObjectCallback(
    std::function<void(Core::GameObject *, const std::string &)> callback) {
  if (m_contextMenu) {
    m_contextMenu->setRenameObjectCallback(callback);
  }
}

void SceneHierarchyWindow::setCreateGameCameraCallback(
    std::function<Core::GameObject *(const std::string &)> callback) {
  if (m_contextMenu) {
...
void InspectorWindow::draw() {
  ...
  if (m_selectedObject) {
    Utils::log_info(std::format("Drawing GameObject Inspector: {}",
...
void InspectorWindow::drawGameObjectInspector() {
  ...
  if (m_scene) {
    const auto *entry =
        m_scene->getRenderBatch().findEntry(m_selectedObject->getId());
    if (entry)
  ...
  auto *scriptComponent =
      m_scene->getComponent<Scripting::LuaScriptComponent>(m_selectedObject);

  // AudioComponentがあれば表示
  auto *audioComponent =
      m_selectedObject->getComponent<Audio::AudioComponent>();
  if (audioComponent) {
  ...
  if (ImGui::CollapsingHeader("Add Component",
    ...
    if (!boxCollider) {
      if (ImGui::Button("Box Collider", ImVec2(-1, 30))) {
        auto *newCollider =
            m_selectedObject->addComponent<Physics::BoxCollider>();
        if (newCollider) {
          newCollider->setSize(Math::Vector3(1.0f, 1.0f, 1.0f));
          Utils::log_info(std::format("BoxCollider added to {}",
                                      m_selectedObject->getName()));
        }
      }
    }
    ...
    if (!audioComponent) {
      if (ImGui::Button("Audio", ImVec2(-1, 30))) {
        auto *newAudio =
            m_scene->addComponent<Audio::AudioComponent>(m_selectedObject);
        if (newAudio) {
          auto initResult = newAudio->initialize();
          if (initResult) {
            Utils::log_info(std::format("AudioComponent added to {}",
                                        m_selectedObject->getName()));
          } else {
  ...
  if (ImGui::CollapsingHeader("Attach Script",
                              ImGuiTreeNodeFlags_DefaultOpen)) {
    if (!scriptComponent) {
      ...
      if (ImGui::BeginDragDropTarget()) {
        if (const ImGuiPayload *payload =
          ...
          if (dropped &&
              dropped->type == static_cast<int>(UI::AssetInfo::Type::Script)) {
            std::string scriptPath = dropped->path;
            std::replace(scriptPath.begin(), scriptPath.end(), '\\', '/');
            std::string lower = scriptPath;
            std::transform(lower.begin(), lower.end(), lower.begin(),
                           ::tolower);
            auto assetsPos = lower.find("assets/");
            if (assetsPos != std::string::npos)
              scriptPath = "Assets/" + scriptPath.substr(assetsPos + 7);

            auto *newScript =
                m_scene->addComponent<Engine::Scripting::LuaScriptComponent>(
                    m_selectedObject, scriptPath);
            if (newScript) {
              Utils::log_info(
                  std::format("Lua script attached: {}", dropped->path));
            }
          }
        }
      ...
      if (ImGui::BeginDragDropTarget()) {
        if (const ImGuiPayload *payload =
          ...
          if (dropped &&
            ...
            m_scene->getComponentBatch().destroyEntity(
                m_selectedObject->getId());
            auto *newScript =
                m_selectedObject
                    ->addComponent<Engine::Scripting::LuaScriptComponent>(
                        dropped->path);
            if (newScript) {
              Utils::log_info(
                  std::format("Lua script replaced: {}", dropped->path));
            }
          }
        }
        ImGui::EndDragDropTarget();
  ...
  if (!scriptComponent && ImGui::BeginDragDropTarget()) {
    if (const ImGuiPayload *payload = ImGui::AcceptDragDropPayload("ASSET")) {
      ...
      if (dropped &&
        ...
        if (newScript) {
          Utils::log_info(
              std::format("Lua script attached (anywhere): {}", dropped->path));
        }
      }
    }
    ImGui::EndDragDropTarget();
...
void InspectorWindow::drawCameraComponent(
...
void InspectorWindow::drawUITextInspector() {
  ...
  if (ImGui::CollapsingHeader("Transform", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::DragFloat3("Position", position, 0.1f)) {
      m_selectedUIText->setPosition(
          Math::Vector3(position[0], position[1], position[2]));
    }

    float rotation[3] = {rot.x, rot.y, rot.z};
    if (ImGui::DragFloat3("Rotation", rotation, 1.0f)) {
      m_selectedUIText->setRotation(
          Math::Vector3(rotation[0], rotation[1], rotation[2]));
    }

    float scaleArr[3] = {scale.x, scale.y, scale.z};
    if (ImGui::DragFloat3("Scale", scaleArr, 0.001f, 0.001f, 10.0f)) {
      m_selectedUIText->setScale(
          Math::Vector3(scaleArr[0], scaleArr[1], scaleArr[2]));
    }
  }
  ...
  if (ImGui::CollapsingHeader("Text Properties",
    ...
    if (ImGui::ColorEdit3("Color", colorArr)) {
      m_selectedUIText->setColor(
          Math::Vector3(colorArr[0], colorArr[1], colorArr[2]));
    }
  ...
  if (gameObject) {
    if (ImGui::CollapsingHeader("Add Component",
      ...
      if (!boxCollider) {
        if (ImGui::Button("Box Collider", ImVec2(-1, 30))) {
          auto *newCollider =
              m_scene->addComponent<Physics::BoxCollider>(m_selectedObject);
          if (newCollider) {
            newCollider->setSize(Math::Vector3(1.0f, 1.0f, 1.0f));
            Utils::log_info(
                std::format("BoxCollider added to {}", gameObject->getName()));
          }
        }
      }
    }
    ...
    auto *scriptComponent =
        gameObject->getComponent<Scripting::LuaScriptComponent>();
        ...
        if (ImGui::BeginDragDropTarget()) {
          if (const ImGuiPayload *payload =
            ...
            if (dropped && dropped->type ==
                               static_cast<int>(UI::AssetInfo::Type::Script)) {
              auto *newScript =
                  gameObject
                      ->addComponent<Engine::Scripting::LuaScriptComponent>(
                          dropped->path);
        ...
        if (ImGui::BeginDragDropTarget()) {
          if (const ImGuiPayload *payload =
            ...
            if (dropped && dropped->type ==
                               static_cast<int>(UI::AssetInfo::Type::Script)) {
              gameObject
                  ->removeComponent<Engine::Scripting::LuaScriptComponent>();
              auto *newScript =
                  gameObject
                      ->addComponent<Engine::Scripting::LuaScriptComponent>(
                          dropped->path);
    ...
    if (!scriptComponent && ImGui::BeginDragDropTarget()) {
      if (const ImGuiPayload *payload = ImGui::AcceptDragDropPayload("ASSET")) {
        ...
        if (dropped &&
            dropped->type == static_cast<int>(UI::AssetInfo::Type::Script)) {
          auto *newScript =
              gameObject->addComponent<Engine::Scripting::LuaScriptComponent>(
                  dropped->path);
          if (newScript) {
            Utils::log_info(std::format("Lua script attached (anywhere): {}",
                                        dropped->path));
          }
        }
      }
...
void InspectorWindow::drawUITextProperties(Engine::EngineUI::UIText *text) {
  ...
  if (ImGui::Button("Remove Component", ImVec2(150, 25))) {
    if (m_scene) {
      ...
      if (selectedObject) {
        ...
        Utils::log_info(
            std::format("UIText removed from {}", selectedObject->getName()));
      }
    }
  }
  ...
  ImGui::Spacing();
  ImGui::TextWrapped(
      "Note: UIText is a 3D object in world space. "
...
void InspectorWindow::drawTransformComponent(Core::Transform *transform) {
  if (ImGui::CollapsingHeader("Transform", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::DragFloat3("Scale", scaleArray, 0.1f, 0.1f, 10.0f)) {
      transform->setScale(
          Math::Vector3(scaleArray[0], scaleArray[1], scaleArray[2]));
    }
  }
}
void InspectorWindow::drawRenderEntry(Core::GameObject *gameObject) {
  ...
  if (ImGui::CollapsingHeader("Render Component",
    ...
    if (ImGui::Combo("Type", &currentType, types, IM_ARRAYSIZE(types))) {
      auto mat =
          entry->material; // entry は unregister 後に無効になるので先に取得
      m_scene->unregisterRenderable(gameObject);
      m_scene->registerRenderable(
          gameObject, static_cast<RenderableType>(currentType), mat);
    }
...
void InspectorWindow::drawMaterialEditor(Core::GameObject *gameObject) {
  ...
  if (ImGui::CollapsingHeader("Material", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::Button("Create New")) {
      std::string newName =
          materialNameBuffer[0] ? materialNameBuffer : "New Material";
    ...
    if (currentMaterial) {
      ...
      if (ImGui::CollapsingHeader("PBR Properties",
        ...
        if (ImGui::CollapsingHeader("Albedo", ImGuiTreeNodeFlags_DefaultOpen)) {
          ...
          ImGui::BeginGroup();
          {
            ...
            ImGui::TextWrapped(currentMaterial->hasTexture(TextureType::Albedo)
                                   ? "Assigned"
                                   : "Drop Texture Here");
            ...
            if (ImGui::BeginDragDropTarget()) {
              if (const ImGuiPayload *payload =
                ...
                if (dropped &&
                    dropped->type ==
                        static_cast<int>(UI::AssetInfo::Type::Texture) &&
                    m_textureManager) {
                  auto tex =
                      m_textureManager->loadTexture(dropped->path, true, true);
                  if (tex) {
                    currentMaterial->setTexture(TextureType::Albedo, tex);
        ...
        if (ImGui::SliderFloat("Metallic", &properties.metallic, 0, 1))
          changed = true;
        if (ImGui::SliderFloat("Roughness", &properties.roughness, 0, 1))
          changed = true;
        if (ImGui::SliderFloat("AO", &properties.ao, 0, 1))
          changed = true;
        if (ImGui::SliderFloat("Alpha", &properties.alpha, 0, 1))
          changed = true;
      }
      ...
      if (ImGui::CollapsingHeader("Emission")) {
        ...
        if (ImGui::SliderFloat("Emissive Strength",
                               &properties.emissiveStrength, 0, 5))
          changed = true;
      }
...
void InspectorWindow::drawBoxColliderComponent(Physics::BoxCollider *collider) {
  if (ImGui::CollapsingHeader("Box Collider", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::DragFloat3("Center", centerArray, 0.1f)) {
      collider->setCenter(
          Math::Vector3(centerArray[0], centerArray[1], centerArray[2]));
    }
    ...
    if (ImGui::DragFloat3("Size", sizeArray, 0.1f, 0.01f, 100.0f)) {
      collider->setSize(
          Math::Vector3(sizeArray[0], sizeArray[1], sizeArray[2]));
    }
    ...
    if (ImGui::Button("Remove Component", ImVec2(150, 25))) {
      if (m_scene) {
        ...
        if (selectedObject) {
          ...
          Utils::log_info(std::format("BoxCollider removed from {}",
...
void InspectorWindow::drawAudioComponent(
  ...
  if (ImGui::CollapsingHeader("Audio Component",
    ...
    if (ImGui::BeginDragDropTarget()) {
      if (const ImGuiPayload *payload = ImGui::AcceptDragDropPayload("ASSET")) {
        ...
        if (dropped) {
          std::string ext =
              std::filesystem::path(dropped->path).extension().string();
    ...
    if (ImGui::Button("Remove Component", ImVec2(150, 25))) {
      if (m_scene) {
        ...
        if (selectedObject) {
          ...
          Utils::log_info(std::format("AudioComponent removed from {}",
                                      selectedObject->getName()));
        }
      }
    }
...
void InspectorWindow::drawTextureSlot(const char *name, TextureType textureType,
  ...
  if (currentTexture) {
    ...
    if (ImGui::IsItemHovered()) {
      ...
      ImGui::Text("Size: %dx%d", currentTexture->getWidth(),
    ...
    if (ImGui::BeginDragDropTarget()) {
      if (const ImGuiPayload *payload = ImGui::AcceptDragDropPayload("ASSET")) {
        const AssetInfo *droppedAsset =
            static_cast<const AssetInfo *>(payload->Data);
        if (droppedAsset->type == AssetInfo::Type::Texture &&
            m_textureManager) {
          auto texture =
              m_textureManager->loadTexture(droppedAsset->path.string());
          if (texture) {
    ...
    if (textureType == TextureType::Albedo) {
      float albedo[3] = {properties.albedo.x, properties.albedo.y,
                         properties.albedo.z};
