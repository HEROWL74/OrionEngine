...
namespace Renderer
{
		...
		D3D12_SHADER_VISIBILITY visiblity)
	{
		...
		*/
		...
		param.DescriptorTable.NumDescriptorRanges = static_cast<UINT>(m_ranges.size() - 1); // Index 仮置き
	...
	RootSignature& RootSignature::addLinearSampler(
		uint32_t shaderRegister,
		D3D12_SHADER_VISIBILITY visiblity)
	{
    ...
    Utils::VoidResult RootSignature::finalize(Device* device)
    {
		...
		CHECK_CONDITION(!m_rootSignature, Utils::ErrorType::ResourceCreation, "RootSignature::finalize() called twice");
