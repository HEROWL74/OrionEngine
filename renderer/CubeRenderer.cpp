...
Utils::VoidResult CubeRenderer::initialize(Device *device) {
  ...
  auto constantBufferResult = m_constantBufferManager.initialize(
      device, Utils::GetRequiredConstantBufferCount());
...
void CubeRenderer::render(const Utils::RenderContext &context) {
  ...
  if (renderCounter++ % 120 == 0) {
    Utils::log_info(std::format(
        "CubeRenderer - View: {}, BufferIndex: {}, Camera: ({:.2f}, {:.2f}, "
        "{:.2f})",
        context.getViewTypeName(), context.getConstantBufferIndex(),
        context.camera->getPosition().x, context.camera->getPosition().y,
  ...
  cameraConstants.viewProjectionMatrix =
      context.camera->getViewProjectionMatrix();
  ...
  objectConstants.worldViewProjectionMatrix =
      context.camera->getViewProjectionMatrix() * m_worldMatrix;
  ...
  if (m_material) {
    ...
    if (tex) {
      context.commandList->SetGraphicsRootDescriptorTable(
          PBRRootSlots::Textures, tex->getSRVHandle());
    }
  }
...
Utils::VoidResult CubeRenderer::createVertexBuffer() {
  ...
  CHECK_HR(m_vertexBuffer->Map(0, &readRange,
...
Utils::VoidResult CubeRenderer::createIndexBuffer() {
  ...
  CHECK_HR(m_indexBuffer->Map(0, &readRange,
...
void CubeRenderer::setupCubeVertices() {
  ...
  m_cubeIndices = {{// Front (CCW: 左下, 右上, 右下)
                    0, 2, 1, 0, 3, 2,
                    // Back
                    4, 5, 6, 4, 6, 7,
                    // Left
...
void CubeRenderer::updateWorldMatrix() {
  ...
  Math::Matrix4 rotationMatrix =
      Math::Matrix4::rotationX(Math::radians(m_rotation.x)) *
